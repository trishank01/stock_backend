...
async function getIntraDayData(symbol, time) {
  ...
  const xmlData = await axios.get(INTRADAY_URL + encodeURIComponent(symbol) + '&Periodicity=' + period + '&PeriodType=' + periodType);
  return new Promise((resolve, reject) => {
    parser(xmlData.data, function (err, result) {
      const data = result.settings.data_sets[0].data_set[0].csv[0].data[0];
      const modifiedData = data.split('\r\n');
      const finalResult = [];
      for (let index = 0; index < modifiedData.length; index++) {
        const tempData = modifiedData[index].split(',');
        const obj = {
          name: tempData[0].split(" ")[1],
          currentTradingPrice: tempData[1] || tempData[2]
        }
